...
export class AlbumsService {
  ...
  create(dto: { name: string; artistId?: string | null }) {
    const item: Album = { id: uuidv4(), name: dto.name, artistId: dto.artistId ?? null };
    this.albums.push(item);
    return item;
  }

  update(id: string, dto: { name: string; artistId?: string | null }) {
